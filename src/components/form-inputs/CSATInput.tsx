      ...
      <div className="flex flex-wrap gap-3 justify-center">
        {Array.from({ length: scale }).map((_, i) => (
          <button
            ...
            disabled={disabled}
            className={`rating-button flex flex-col items-center gap-2 p-4 rounded-lg border-2 min-w-[80px] ${
              value === i + 1
                ? "rating-button-selected bg-primary/10 text-primary border-primary"
                : "border-border bg-background hover:border-primary hover:bg-accent"
            } ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            <span className="text-3xl">{emojis[scale][i]}</span>
            <span className="text-xs text-center font-medium">{labels[i]}</span>
          </button>
        ))}
      </div>
    </div>
